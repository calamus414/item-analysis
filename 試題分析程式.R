...
ui <- fluidPage(
  ...
  sidebarPanel(fileInput("file1", "選擇檔案", accept = ".xlsx"),
               numericInput("point", "每題分數", 5, min = 1, max = 100),
               actionButton("writedown", "輸出"),
  ),
  mainPanel(
    tabsetPanel(
...
server <- function(input, output) {
  ...
  observeEvent(input$writedown,{
    ...
    itemTable<-cbind(itemTable,correct,a,b,c,d)
    write.csv(itemTable,'Item Analysis.csv')
    ...
    write.csv(resulttable,'Correcting.csv')
